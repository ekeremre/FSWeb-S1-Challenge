...
function indekseGoreFenomen(dizi, no) {
  return `${no}. indekste bulunan fenomen: ${dizi[no]["profile"]}`;
}
...
function profilListesi(dizi1) {
  const newdizi = [...dizi1];
  let newdizi2 = [];
  for (const item of newdizi) {
    newdizi2.push(item["profile"]);
  }
  return newdizi2;
}
...
function fenomenSil(dizi2, no2) {
  const newdizi = [...dizi2];
  newdizi.splice(no2, 1);
  return newdizi;
}
console.log("yeni array:", fenomenSil(fenomenler, 1));
/* Görev 6:
...
function fenomenEkle(dizi3, sirano, profile1, follower1, post1, platform1) {
  const newdizi = [...dizi3];
  newdizi.push({
    number: sirano,
    profile: profile1,
...
function enFenomenler(dizi4) {
  const fenomenler = [];
  for (const item of dizi4) {
    if (item["followers"] > 100000000) {
      fenomenler.push(item["profile"]);
    }
  }
  return fenomenler;
}
console.log("populerler:", enFenomenler(fenomenler));
